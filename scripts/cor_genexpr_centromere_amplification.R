...
# inputs
ca_scores_file = file.path(PREP_DIR,'centrosome_amplification.tsv')
...
# outputs
correlations_file = file.path(RESULTS_DIR,'files','correlation-genexpr_centrosome_amplification.tsv')

# load data (NOTE: gene expression data is loaded on the fly)
score = read_tsv(ca_scores_file) 
metadata = read_tsv(phenotype_file)
# merge(metadata, score, by='sample') %>% count(cancer_type,sample_type)
...
sample_types = metadata %>% pull(sample_type) %>% unique()

cl =  makeCluster(N_CORES)
registerDoParallel(cl)
correlations = foreach(cancer_type_oi=cancer_types, .combine='cbind') %:%
                foreach(sample_type_oi=sample_types,
                        .packages=c('dplyr','limma'),
                        .combine='cbind') %dopar% {
    # get samples for correlation
    samples_oi = metadata %>% filter(cancer_type==cancer_type_oi & sample_type==sample_type_oi) %>% pull(sample)
    ...
    y = score$CA20[match(samples_oi, score$sample)]
    corr = cor(x, y, method=COR_METHOD)    
    colnames(corr) = paste0(cancer_type_oi,'_',gsub(' ','',sample_type_oi))
    return(corr)
}
stopCluster(cl)
